...
AltIDirectDrawSurface_blt(p1, p2, p3, p4, p5, p6)
{
	...
	if (g_.target_name = "JK.exe")
	{
		lvl         := numget(0x552b80+0, "uchar")
		if ( (lvl >= 15) && (last < 15) )
		{
			logerr("Change to disk 2")
			dllcall(g_.p.SndMsg, uint, g_.cfg.script_hwnd, uint, dllcall("RegisterWindowMessageW", str, "Peixoto.MCI"), uint, 2, uint, 1)
		}
		else if  ( (lvl < 15) && (last >= 15) )
		{
			logerr("Change to disk 1")
			dllcall(g_.p.SndMsg, uint, g_.cfg.script_hwnd, uint, dllcall("RegisterWindowMessageW", str, "Peixoto.MCI"), uint, 2, uint, 0)
		}		
		last := lvl	
	}
